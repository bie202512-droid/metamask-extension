...
describe('HomeDeepLinkActions', () => {
  ...
  it('notifies Home to show the rewards QR code for a rewards deeplink URL', async () => {
    const deeplinkUrl =
      'https://link.metamask.io/rewards?referral=ABC123&sig_params=referral&sig=signature&utm_source=twitter';
    const onQrCodeDeepLink = jest.fn();
    const { Wrapper } = createWrapper({
      pathname: DEFAULT_ROUTE,
      search: `?${new URLSearchParams({
        [HomeQueryParams.RewardsDeeplinkUrl]: deeplinkUrl,
      }).toString()}`,
      isNetworkMenuOpen: false,
    });
  ...
  it('ignores QR deeplink params that do not point to the expected route', () => {
    const onQrCodeDeepLink = jest.fn();
    const { Wrapper } = createWrapper({
      pathname: DEFAULT_ROUTE,
      search: `?${new URLSearchParams({
